	...
	function mirasat_generate_css() {
		...
		if ( function_exists( 'FW' ) ) {
			...
			if (! empty( $header_bg ) OR $featured_bg == 'enabled'  ) {
				...
				if ( has_post_thumbnail() && $featured_bg == 'enabled' && get_post_type() != 'portfolio') {
			...
			if ( get_post_type() == 'portfolio' ) {
				...
				$alt_bg = fw_get_db_post_option( $wp_query->get_queried_object_id(), 'header_bg' );

				if ( !empty($alt_bg) ) {

					$theme_style .= '.page-header { background-image: url(' . esc_url( $alt_bg['url'] ) . ') !important; } ';
				}
